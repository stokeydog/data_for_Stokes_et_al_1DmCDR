...
path_for_config_high_mixing = "results/data/examples/CN_1D_examples/finite_ELD_1m_high_mixing_test_doc/diffusivity_1eminus02/results_dilution_100.jld2"
path_for_config_med_mixing = "results/data/examples/CN_1D_examples/finite_ELD_1m_med_mixing_test_doc/diffusivity_1eminus05/results_dilution_100.jld2"
...
target_depth = sqrt.(data["input"]["config"]["ocn_props"]["vertical_diffusivity_ML"] .* data["output"]["TI"])
idx = map(td -> argmin(abs.(data["output"]["z"] .- td)), target_depth)

t   = data["output"]["TI"]                     # time vector (Nt)
Cs  = data["output"]["DIC_p"][end, :]          # surface row (Nt)
DIC = data["output"]["DIC_p"]                  # Nz × Nt array

# element-wise pick: (row = idx[i], col = i)
Cz  = getindex.(Ref(DIC), idx, eachindex(idx)) # Nt-vector

flux_int_1 = 2 ./ t .* ( Cs .- Cz ) .* 1e-6      # µmol → mol
flux_int = ( Cs .- Cz ) .* 1e-6      # µmol → mol
flux_int[1] = 0
...
flux_int = 2 ./ t .* ( Cs .- Cz ) .* 1e-6      # µmol → mol
flux_int[1] = 0

p2 = plot( data["output"]["TI"] ./ 86400, flux_surf./abs.(flux_int); 
        size = (800,500),
        label = "\$\\kappa = 10^{-2}\$",
        xlabel = "time (days)", 
        ylabel = "Biot number",
