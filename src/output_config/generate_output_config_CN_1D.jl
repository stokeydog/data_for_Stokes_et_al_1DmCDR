...
"""
function generate_output_config_CN_1D(save_interval_hours::Int, dt::Float64, sim_duration_days::Float64)::OutputConfigCN1D
    save_interval_seconds = save_interval_hours * 3600.0
    ind_save = ceil(Int, save_interval_seconds / dt)
    Tend = sim_duration_days * 86400.0  # Convert days to seconds
    TI = 0:ind_save:ceil(Int, Tend / dt)
    NT = length(TI)
