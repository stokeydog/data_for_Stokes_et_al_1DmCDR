...
# use this for infinite MLD, 10m ELD
file_paths = [
#     "./results/inf_MLD_1m_ELD_doc/diffusivity_1eminus03/results_dilution_0.jld2",
...
color_palette = Plots.palette(:coolwarm, length(file_paths))  # Choose a palette with enough colors
...
vline!(plt[1], [1e-7 * (2040/(0.0375*326))^2 / (1 * (4.17e-5)^2) / 86400], 
    linestyle=:dash, 
...
vline!(plt[1], [1e-8 * (2040/(0.0375*326))^2 / (1 * (4.17e-5)^2) / 86400], 
    linestyle=:dash, 
...
for (i, file_path) in enumerate(file_paths)
    ...
    plot!(plt[2], TI / 86400, ΔpCO2, label=label_text, color=color_palette[i],
        ...
        # xlims=[0,0.1],ylims=[-75,-68],
        linewidth=2, legend=:bottomright,
        xlims=[0,30]
        )
end
# vline!(plt[2], [1e-6 * (2040/(0.0375*326))^2 / (1 * (4.17e-5)^2) / 86400], 
#     linestyle=:dash, 
#     color=color_palette[1], 
#     linewidth=2, 
#     label="t = \\kappa = 10^{-7}"
# )
vline!(plt[2], [1e-7 * (2040/(0.0375*326))^2 / (1 * (4.17e-5)^2) / 86400], 
    linestyle=:dash, 
    color=color_palette[1], 
    linewidth=2, 
    # label="t = \\kappa = 10^{-7}",
...
vline!(plt[2], [1e-8 * (2040/(0.0375*326))^2 / (1 * (4.17e-5)^2) / 86400], 
    linestyle=:dash, 
    color=color_palette[2], 
    linewidth=2, 
    # label="t = \\kappa = 10^{-7}",
...
#         linewidth=2, legend=:topright)
# end
# vline!(plt[3], [1e-7 * (2040/(0.0375*326))^2 / (1 * (4.17e-5)^2) / 86400], 
#     linestyle=:dash, 
#     color=color_palette[1], 
#     linewidth=2, 
#     label="t = \\kappa = 10^{-7}",
#     xlims=[0,30]
# )
# vline!(plt[3], [1e-8 * (2040/(0.0375*326))^2 / (1 * (4.17e-5)^2) / 86400], 
#     linestyle=:dash, 
#     color=color_palette[2], 
#     linewidth=2, 
#     label="t = \\kappa = 10^{-7}",
...
# savefig(plt, "additionality_10cmELD_kg6mps.png")


# ## DIAGNOSTIC PLOTS 

# # Heatmap of DIC_p over time and depth
# DIC, pCO2, dpco2, TI, z, additionality, additionality_flux, additionality_dic = load_simulation_data("./results/inf_MLD_Bic_range_u6p63_1mEffLayer_doc/diffusivity_3eminus06/results_dilution_100.jld2") # load data for heatmap
